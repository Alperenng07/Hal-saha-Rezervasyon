...
export default function WeeklyCalendar({
  ...
  return (
    <div className="bg-white/80 backdrop-blur-xl rounded-xl sm:rounded-[1.4rem] overflow-hidden relative ring-1 ring-slate-900/5 max-w-full">
      <div className="p-2 sm:p-3 md:p-6 border-b border-slate-100 flex flex-col gap-2 sm:gap-3 md:gap-4 md:flex-row md:justify-between md:items-center bg-white/50">
        <div className="flex gap-1.5 sm:gap-2 w-full md:w-auto p-1 bg-slate-100/80 rounded-lg sm:rounded-xl border border-slate-200/50 overflow-x-auto scroll-touch min-w-0">
          {pitches.map((p) => (
            <button
              key={p.id}
        ...
        <div className="flex items-center justify-between w-full md:w-auto gap-1.5 sm:gap-4 bg-white md:bg-transparent p-1.5 sm:p-3 md:p-0 rounded-lg sm:rounded-xl border border-slate-200/50 md:border-0 shadow-sm md:shadow-none min-w-0">
          <button
            onClick={handlePrevWeek}
            className="shrink-0 p-1.5 sm:p-2 text-slate-400 rounded-lg nav-icon-btn"
            aria-label="Önceki hafta"
          >
            ←
          </button>
          <div className="flex flex-col items-center min-w-0 flex-1 px-1">
            <span className="font-bold text-slate-900 text-center text-[11px] sm:text-sm md:text-base tracking-tight leading-tight">
              {format(startOfCurrentWeek, "d MMM", { locale: tr })} -{" "}
              {format(addDays(startOfCurrentWeek, 6), "d MMM", { locale: tr })}
            </span>
            <span className="text-[9px] sm:text-xs font-semibold text-brand uppercase tracking-wider">
      ...
      <div className="md:hidden px-2 sm:px-3 pb-3 sm:pb-4 min-w-0">
        <div className="flex gap-1.5 sm:gap-2 overflow-x-auto py-2 sm:py-3 scroll-touch min-w-0">
          {weekDays.map((day) => {
            ...
            return (
              <button
                ...
                className={`shrink-0 flex flex-col items-center min-w-[2.65rem] sm:min-w-[3.25rem] px-1.5 sm:px-2 py-1.5 sm:py-2 rounded-xl border day-pill ${
                  isSelected
                    ? "day-pill-selected"
                    : isToday
                      ? "day-pill-today"
              ...
              >
                ...
                <span className="text-sm sm:text-base font-bold leading-tight">{format(day, "d")}</span>
              </button>
            );
          })}
        </div>

        <p className="text-[11px] sm:text-xs text-slate-500 mb-2 px-0.5 truncate">
        ...
        {slots.length === 0 ? (
          ...
          <div className="space-y-1.5 sm:space-y-2 max-h-[50vh] overflow-y-auto scroll-touch">
            {slots.map((slot) => {
              ...
              return (
                <div
                  key={slot.startTime}
                  className="flex items-center gap-1.5 sm:gap-2 rounded-xl border border-slate-100 bg-white p-2 sm:p-2.5 shadow-sm min-w-0"
                >
                  ...
                  {booked ? (
                    <div className="flex-1 min-w-0 py-2 sm:py-2.5 bg-rose-50 border border-rose-100 text-rose-600 rounded-lg text-center font-bold text-[11px] sm:text-xs">
                      DOLU
                    </div>
                  ) : subscribed ? (
                    <div className="flex-1 min-w-0 py-2 sm:py-2.5 bg-blue-50 border border-blue-100 text-blue-700 rounded-lg text-center font-bold text-[11px] sm:text-xs">
                      ABONE
                    </div>
                  ) : isPast ? (
                    <div className="flex-1 min-w-0 py-2 sm:py-2.5 bg-slate-50 text-slate-400 rounded-lg text-center font-medium text-[11px] sm:text-xs border border-dashed border-slate-200">
                      Geçti
                    </div>
                  ) : (
                    <button
                      ...
                      className="flex-1 min-w-0 py-2 sm:py-2.5 rounded-lg font-bold text-[11px] sm:text-xs slot-available"
                    >
                      SEÇ · {slot.endTime}
                    </button>
                  )}
